...
try {
    ...
    $result = $emailManager->getArchivedEmailsForUser((int) $_SESSION['admin_user_id']);

    if (!$result['success']) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => $result['error'] ?? 'Unable to load archived emails']);
        exit;
    }

    echo json_encode(['success' => true, 'emails' => $result['emails'] ?? []]);
} catch (Exception $e) {
    logError('get archived emails handler error: ' . $e->getMessage());
